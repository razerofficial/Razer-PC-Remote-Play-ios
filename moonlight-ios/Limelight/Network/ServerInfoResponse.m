...
- (void) populateHost:(TemporaryHost*)host {
    ...
    host.machineIdentifier = [[self getStringTag:TAG_MACHINE_IDENTIFIER] trim];
    ...
    if ([self getIntTag:TAG_HTTPS_PORT value:&httpsPort]) {
        ...
        host.httpsPort = RZ_DEFAULT_HTTPS_PORT;
    }
...
//    }
    ...
    Log(LOG_I, @"%s Host: %@ : %@ pairState:%d", __func__, host.name, host.localAddress, host.pairState);
