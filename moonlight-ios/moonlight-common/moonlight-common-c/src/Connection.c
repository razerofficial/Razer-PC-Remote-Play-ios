...
//        err = resolveHostName(serverInfo->address, AF_UNSPEC, 47984, &RemoteAddr, &AddrLen);
        err = resolveHostName(serverInfo->address, AF_UNSPEC, 51332, &RemoteAddr, &AddrLen);
        if (err != 0) {
//            err = resolveHostName(serverInfo->address, AF_UNSPEC, 47989, &RemoteAddr, &AddrLen);
            err = resolveHostName(serverInfo->address, AF_UNSPEC, 51337, &RemoteAddr, &AddrLen);
        }
        if (err != 0) {
//            err = resolveHostName(serverInfo->address, AF_UNSPEC, 48010, &RemoteAddr, &AddrLen);
            err = resolveHostName(serverInfo->address, AF_UNSPEC, 51358, &RemoteAddr, &AddrLen);
        }
    }
    if (err != 0) {
        Limelog("failed: %d\n", err);
