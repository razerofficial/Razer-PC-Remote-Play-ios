...
@interface DiscoveryManager ()<DiscoveryWorkerDelegate>
...
- (void) discoverHost:(NSString *)hostAddress withCallback:(void (^)(TemporaryHost *, NSString*))callback {
    ...
    NSString* prohibitedAddressMessage = [NSString stringWithFormat: @"Razer PC Remote Play only supports adding PCs on your local network on %s.",
    #if TARGET_OS_TV
                                   "tvOS"
    #else
                                   "iOS"
...
- (void)updateHost:(TemporaryHost*)host {
    ...
    //    TemporaryHost *addHost = host;
    //    for (TemporaryHost *old in _hostQueue) {
    ...
    //            old.state = StateOnline;
    //            [[ShareDataDB shared] updateHost:old];
    //            addHost = old;
    //
    //            [worker cancel];
    //
    //            [[SettingsRouter shared].dataManager removeHost:host];
    //            return;
    //        }
    //    }
...
- (NSOperation*) createWorkerForHost:(TemporaryHost*)host {
    DiscoveryWorker* worker = [[DiscoveryWorker alloc] initWithHost:host uniqueId:_uniqueId delegate:self];
    return worker;
}

#pragma mark -- DiscoveryWorkerDelegate

- (void)removeDuplicatedHost:(TemporaryHost *)host {
    @synchronized (_hostQueue) {
        NSMutableArray *sameWorkers = [NSMutableArray new];
        DiscoveryWorker *match;
        for (DiscoveryWorker* worker in [_opQueue operations]) {
            if ([worker getHost] == host) {
                match = worker;
            } else if (host.serverCert.length > 0 && [[worker getHost].machineIdentifier isEqualToString:host.machineIdentifier]) {
                [sameWorkers addObject:worker];
            }
        }
        if ([sameWorkers count] > 0) {
            [match cancel];
            for (DiscoveryWorker *worker in sameWorkers) {
                [worker getHost].serverCert = host.serverCert;
            }
        }
        ...
        [_pausedHosts removeObject:host];
        [[SettingsRouter shared].dataManager removeHost:host];
        [[ShareDataDB shared] removePairedHostFromeShareDB:host];
