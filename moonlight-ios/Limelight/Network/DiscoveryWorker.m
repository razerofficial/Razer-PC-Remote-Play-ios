...
@property id<DiscoveryWorkerDelegate> delegate;
@end
        ...
        for (NSString *address in addresses) {
            ...
            if ([self checkIsDuplicatedHost:serverInfoResp]) {
                if ([_delegate respondsToSelector:@selector(removeDuplicatedHost:)]) {
                    [_delegate removeDuplicatedHost:_host];
                }
                break;
            }
...
- (BOOL) checkIsDuplicatedHost:(ServerInfoResponse*)response {
    if ([response isStatusOk]) {
        // If the response is from same host
        if (![[response getStringTag:TAG_UNIQUE_ID].trim isEqualToString:_host.uuid] && [[response getStringTag:TAG_MACHINE_IDENTIFIER] isEqualToString:_host.machineIdentifier]) {
            Log(LOG_I, @"Received response from incorrect host: %@ expected: %@ machineIdentifier: %@", [response getStringTag:TAG_UNIQUE_ID], _host.uuid, _host.machineIdentifier);
            return YES;
        }
    }
    return NO;
