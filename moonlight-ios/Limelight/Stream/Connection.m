...
int DrDecoderSetup(int videoFormat, int width, int height, int redrawRate, void* context, int drFlags)
{
    ...
    ExtraConfig *config = [ExtraConfig new];
    config.width = width;
    config.height = height;
    config.activeVideoFormat = activeVideoFormat;
    [[DevUtils shared] setupStats:config];
    return 0;
}
...
void DrStop(void)
{
    [[DevUtils shared] saveStatsToShareDB];
    [renderer stop];
}
...
-(BOOL) getVideoStats:(video_stats_t*)stats
{
    ...
    if (lastVideoStats.endTime != 0) {
        [[DevUtils shared] update:currentVideoStats last:lastVideoStats];
