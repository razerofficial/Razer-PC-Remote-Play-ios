...
- (void)viewDidLoad
{
    ...
    [_stageLabel setText:[NSString stringWithFormat:Localized(@"Starting %@"), self.streamConfig.appName]];
...
#else
    [_tipLabel setText:Localized(@"Tip: Swipe from the left edge to disconnect from your PC")];
#endif
...
- (void) connectionStarted {
    ...
    dispatch_async(dispatch_get_main_queue(), ^{
        ...
        if (self->_settings.statsOverlay || [DevUtils shared].isDebugMode) {
...
//    unsigned int portTestResults = LiTestClientConnectivity(CONN_TEST_SERVER, 443, portFlags);
    unsigned int portTestResults = 0;
    ...
    dispatch_async(dispatch_get_main_queue(), ^{
        ...
        if (portTestResults != ML_TEST_RESULT_INCONCLUSIVE && portTestResults != 0) {
            ...
            message = Localized(@"Your device's network connection is blocking Razer PC Remote Play. Streaming may not work while connected to this network.");
        }
        else {
            switch (errorCode) {
                ...
                case ML_ERROR_NO_VIDEO_TRAFFIC:
                    title = Localized(@"Connection Error");
                    message = Localized(@"No video received from host.");
                    break;
                    
                case ML_ERROR_NO_VIDEO_FRAME:
                    title = Localized(@"Connection Error");
                    message = Localized(@"Your network connection isn\'t performing well. Reduce your video bitrate setting or try a faster connection.");
                    break;
                    
                case ML_ERROR_UNEXPECTED_EARLY_TERMINATION:
                case ML_ERROR_PROTECTED_CONTENT:
                    title = Localized(@"Connection Error");
                    message = Localized(@"Something went wrong with your host PC when starting the stream.\n\nMake sure you don\'t have any DRM-protected content open on your host PC. You can also try restarting your host PC.");
                    break;
                    
                case ML_ERROR_FRAME_CONVERSION:
                    title = Localized(@"Connection Error");
                    message = Localized(@"The host PC reported a fatal video encoding error.\n\nTry disabling HDR mode, changing the streaming resolution, or changing your host PC\'s display resolution.");
                    break;
                ...
                default:
                {
                    ...
                    title = Localized(@"Connection Terminated");
                    message = [NSString stringWithFormat:@"%@\n\n%@: %@", Localized(@"Error code"),Localized(@"The connection was terminated"),errorString];
                    break;
                }
            }
        }
        ...
        if (portFlags != 0) {
            ...
            message = [message stringByAppendingString:[NSString stringWithFormat:@"\n\n%@\n%s", Localized(@"Check your firewall and port forwarding rules for port(s):"),failingPorts]];
        }
        ...
        [conTermAlert addAction:[UIAlertAction actionWithTitle:Localized(@"OK") style:UIAlertActionStyleDefault handler:^(UIAlertAction* action){
            [self returnToMainFrame];
        }]];
...
//    unsigned int portTestResults = LiTestClientConnectivity(CONN_TEST_SERVER, 443, portTestFlags);
    ...
    dispatch_async(dispatch_get_main_queue(), ^{
        ...
        NSString* message = [NSString stringWithFormat:Localized(@"%s failed with error %d"), stageName, errorCode];
        if (portTestResults != ML_TEST_RESULT_INCONCLUSIVE && portTestResults != 0) {
            message = [message stringByAppendingString:[NSString stringWithFormat:@"\n\n%@",Localized(@"Your device's network connection is blocking Razer PC Remote Play. Streaming may not work while connected to this network.")]];
        }
        ...
        UIAlertController* alert = [UIAlertController alertControllerWithTitle:Localized(@"Connection Failed")
                                                                       message:message
                                                                preferredStyle:UIAlertControllerStyleAlert];
        ...
        switch (status) {
            ...
            case CONN_STATUS_POOR:
                if (self->_streamConfig.bitRate > 5000) {
                    [self updateOverlayText:Localized(@"Slow connection to PC\nReduce your bitrate")];
                }
                else {
                    [self updateOverlayText:Localized(@"Poor connection to PC")];
                }
                break;
        }
    });
