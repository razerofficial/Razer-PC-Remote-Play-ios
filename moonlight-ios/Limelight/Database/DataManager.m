...
- (void) saveSettingsWithBitrate:(NSInteger)bitrate
               ...
               absoluteTouchMode:(BOOL)absoluteTouchMode
                    statsOverlay:(BOOL)statsOverlay
                       hostFramerate:(NSInteger)hostFramerate
                          hostHeight:(NSInteger)hostHeight
                           hostWidth:(NSInteger)hostWidth {
    ...
    [_managedObjectContext performBlockAndWait:^{
        ...
        settingsToSave.absoluteTouchMode = absoluteTouchMode;
        settingsToSave.statsOverlay = statsOverlay;
        settingsToSave.hostWidth = [NSNumber numberWithInteger:hostWidth];;
        settingsToSave.hostHeight = [NSNumber numberWithInteger:hostHeight];;
        settingsToSave.hostFramerate = [NSNumber numberWithInteger:hostFramerate];
        [self saveData];
    }];
}

- (void) updateHost:(TemporaryHost *)host {
    Log(LOG_I, @"updateHost to database: %@ pairState:%d", host.name, host.pairState);
    [_managedObjectContext performBlockAndWait:^{
