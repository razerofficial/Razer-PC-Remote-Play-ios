...
- (void)startPairing:(NSString *)PIN {
    ...
    dispatch_sync(dispatch_get_main_queue(), ^{
        self->_pairAlert = [UIAlertController alertControllerWithTitle:@"Pairing"
                                                               message:[NSString stringWithFormat:@"Enter the following PIN on the host machine: %@\n\nIf your host PC is running Razer Cortex, navigate to the Razer Cortex UI to enter the PIN.", PIN]
            ...
            dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
                ...
                dispatch_sync(dispatch_get_main_queue(), ^{
                    [self hideLoadingFrame:^{
                        ...
                        if (portTestResult == 0) {
                            message = @"This network does not appear to be blocking Razer PC Remote Play. If you still have trouble connecting, check your PC's firewall settings.\n\nVisit the Razer PC Remote Play Setup Guide on GitHub for additional setup help and troubleshooting steps.";
                        }
                        else if (portTestResult == ML_TEST_RESULT_INCONCLUSIVE) {
                            message = @"The network test could not be performed because none of Razer PC Remote Play's connection testing servers were reachable. Check your Internet connection or try again later.";
                        }
                        else {
                            char blockedPorts[512];
                            LiStringifyPortFlags(portTestResult, "\n", blockedPorts, sizeof(blockedPorts));
                            message = [NSString stringWithFormat:@"Your current network connection seems to be blocking Razer PC Remote Play. Streaming may not work while connected to this network.\n\nThe following network ports were blocked:\n%s", blockedPorts];
                        }
...
- (void) addHostClicked {
    ...
    UIAlertController* alertController = [UIAlertController alertControllerWithTitle:@"Add Host Manually" message:@"If Razer PC Remote Play doesn't find your local gaming PC automatically,\nenter the IP address of your PC" preferredStyle:UIAlertControllerStyleAlert];
